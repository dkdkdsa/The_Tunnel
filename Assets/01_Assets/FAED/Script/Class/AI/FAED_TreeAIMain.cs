    ...
    public class FAED_TreeAIMain
    {
        ...
        public FAED_TreeAIMain(FAED_TreeData data, FAED_TreeAI aI)
        {
            ...
            foreach(var item in data.nodes)
            {
                ...
                if(item.type == FAED_TreeNodeType.Root)
                {
                ...
                else if(item.type == FAED_TreeNodeType.Sequence)
                {

                    obj.AddComponent<FAED_Sequence>();

                }
                else if(item.type == FAED_TreeNodeType.If)
                {

                    obj.name = $"{obj.name}(If)";

                }
                else if (item.type == FAED_TreeNodeType.Tree)
                {
        ...
        public void StartAI()
        {
            ...
            foreach(var item in data.nodes)
            {
                ...
                if (item.type == FAED_TreeNodeType.Sequence)
                {
                ...
                if (item.type == FAED_TreeNodeType.If)
                {
        ...
        private void FindAllNode<T>(T root, string guid) where T : FAED_TreeAINode, IFAED_StateTreeNode<FAED_TreeAINode>
        {
            ...
            if (data.links.Find(x => x.baseGUID == guid) != null)
            {
                ...
                if (data.nodes.Find(x => x.GUID == guid).type == FAED_TreeNodeType.Sequence) return;
                if (data.nodes.Find(x => x.GUID == guid).type == FAED_TreeNodeType.If) return;
